...
func (m *Manager) download(anime *models.AnimeEntity) {
	...
	if status, has := m.name2status[name]; has {
		...
		if status.State != StateNotFound {
			// 文件已存在
			if len(status.Path) != 0 && utils.IsExist(xpath.Join(manager.DownloaderConf.SavePath, status.Path)) {
...
func (m *Manager) UpdateDownloadItem(status *models.DownloadStatus, anime *models.AnimeEntity, item *models.TorrentItem) {
	...
	if !status.Init {
		...
		renameOpt := &RenameOptions{
			Src:   xpath.Join(manager.DownloaderConf.DownloadPath, content.Name),
			Dst:   xpath.Join(manager.DownloaderConf.SavePath, renamePath),
...
func (m *Manager) scrape(bangumi *models.AnimeEntity) bool {
	nfo := xpath.Join(manager.DownloaderConf.SavePath, bangumi.DirName(), "tvshow.nfo")
